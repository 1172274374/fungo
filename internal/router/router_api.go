...
import (
	...
	"github.com/xinliangnote/go-gin-api/internal/api/codetest"
	...
	"github.com/xinliangnote/go-gin-api/task"
)

func setApiRouter(r *resource) {

	//开始定时任务
	if configs.TurnOnTask == true {
		go task.StartTask(r.db, r.cache)
	}
	...
	login := r.mux.Group("/api", r.interceptors.CheckSignature())
	{
		...
		// codeTest
		codeTestHandler := codetest.New(r.logger, r.db, r.cache)
		login.POST("/uploadFile", codeTestHandler.Create())
		login.DELETE("/testDelete/:id", codeTestHandler.Delete())
		login.PATCH("/testPatch", codeTestHandler.Update())
